...
public class PanelArrastrable : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    [Tooltip("Escala al agarrar el panel, 1.05 = 5% mas grande")]
...
void Awake() { if (objetivo == null) objetivo = transform as RectTransform; var canvas = GetComponentInParent<Canvas>(); if (canvas != null) { canvasRT = canvas.transform as RectTransform; camaraUI = canvas.renderMode == RenderMode.ScreenSpaceOverlay ? null : canvas.worldCamera; } escalaOriginal = objetivo != null ? objetivo.localScale : Vector3.one; escalaObjetivo = escalaOriginal; }

public void OnPointerDown(PointerEventData eventData) { if (objetivo == null) return; arrastrando = true; escalaObjetivo = escalaOriginal * escalaAgarre; if (AudioManager.Instance != null) AudioManager.Instance.SonarClick(); Vector2 cursorLocal; RectTransformUtility.ScreenPointToLocalPointInRectangle(canvasRT, eventData.position, camaraUI, out cursorLocal); offsetArrastre = objetivo.anchoredPosition - cursorLocal; }
...
public void OnPointerUp(PointerEventData eventData) { if (arrastrando && AudioManager.Instance != null) AudioManager.Instance.SonarClick(); arrastrando = false; escalaObjetivo = escalaOriginal; }
    ...
    void Update() { if (objetivo == null) return; objetivo.localScale = Vector3.Lerp(objetivo.localScale, escalaObjetivo, Time.deltaTime * velocidadEscala); }
}
