...
public void Jugar() { StartCoroutine(JugarConFadeMusica()); }

    [Tooltip("Duracion del fade out de la musica antes de cambiar de escena")]
    public float duracionFadeMusica = 1f;

    IEnumerator JugarConFadeMusica() { if (AudioManager.Instance != null) yield return StartCoroutine(AudioManager.Instance.FadeOutMusica(duracionFadeMusica)); if (GameManager.Instance != null) GameManager.Instance.IniciarJuego(); }
