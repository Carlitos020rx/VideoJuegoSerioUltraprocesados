...
public class ArrastreCuaderno : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    [Header("Efecto de agarre")]
    [Tooltip("Escala al agarrar el cuaderno, 1.05 = 5% mas grande")]
    public float escalaAgarre = 1.05f;
...
void Awake() { var canvas = GetComponentInParent<Canvas>(); if (canvas != null) { canvasRT = canvas.transform as RectTransform; camaraUI = canvas.renderMode == RenderMode.ScreenSpaceOverlay ? null : canvas.worldCamera; } escalaOriginal = objetivoArrastrar != null ? objetivoArrastrar.localScale : Vector3.one; escalaObjetivo = escalaOriginal; }

public void OnPointerDown(PointerEventData eventData) { if (objetivoArrastrar == null) return; arrastrando = true; escalaObjetivo = escalaOriginal * escalaAgarre; if (AudioManager.Instance != null) AudioManager.Instance.SonarClick(); Vector2 cursorLocal; RectTransformUtility.ScreenPointToLocalPointInRectangle(canvasRT, eventData.position, camaraUI, out cursorLocal); offsetArrastre = objetivoArrastrar.anchoredPosition - cursorLocal; }
...
public void OnPointerUp(PointerEventData eventData) { if (arrastrando && AudioManager.Instance != null) AudioManager.Instance.SonarClick(); arrastrando = false; escalaObjetivo = escalaOriginal; }

    void Update() { if (objetivoArrastrar == null) return; objetivoArrastrar.localScale = Vector3.Lerp(objetivoArrastrar.localScale, escalaObjetivo, Time.deltaTime * velocidadEscala); }
}
